...
class Fams(tornado.websocket.WebSocketHandler):
    ...
    def open(self):  # WebSocket建立时调用
        ...
        self.openid = openid
        Fams.clients[openid] = {"openid": openid, "object": self}
        logging.info("User {0} connected".format(openid))

    def on_close(self):  # WebSocket断开连接时调用
        if self.openid in Fams.clients:
            del Fams.clients[self.openid]
...
def main():
    ...
    app = tornado.web.Application([(r"/", Fams)])
